...
public class TimeBroadcastPacket {
    ...
    public TimeBroadcastPacket(long day, DayTime dayTimeID,long dayTime) {
            ...
            if (context.getDirection() == NetworkDirection.PLAY_TO_CLIENT) {
                switch (displayType) {
                    case CENTER_SUBTITLE -> {
                        ...
                        Component subTitle = Component.translatable("daytime.zombiegamereborn." + dt.id).append("§l | §r").append(DayTime.transformToTime(this.dayTime)).withStyle(ChatFormatting.GRAY);
                    ...
                    case CHAT_MESSAGE -> {
                        if (!chatComponentJson.isEmpty()) {
                            ...
                            if (chatMsg != null && Minecraft.getInstance().player != null) {
                                Minecraft.getInstance().player.displayClientMessage(chatMsg, false);
                            }
                        }
                    }
                }
